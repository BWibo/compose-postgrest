...
- **PostgREST** — connects as `postgrest_user` (unprivileged) and exposes the schema defined by `DB_SCHEMA` (default: `testschema`) in the `postgrest` database as a REST API.
...
| `POSTGREST_USER` / `POSTGREST_PASSWORD` | `postgrest_user` / `postgrest_pass` | Unprivileged user PostgREST connects as |
...
2. Creates `${DB_ANON_ROLE}` as a NOLOGIN role and `${POSTGREST_USER}` as a LOGIN role with password
3. Grants `${DB_ANON_ROLE}` to `${POSTGREST_USER}` (required for PostgREST role switching)
